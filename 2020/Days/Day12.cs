...
namespace AOC2020.Days
{
	public class Day12 : Christmas
	{
		...
		public override string Level1(string[] input)
		{
			...
			position = Instructions.Aggregate(
				position,
				(currentLocation, instruction) => instruction.action switch
					{
						"F" => new coord(currentLocation.x + currentLocation.dir.Item1 * instruction.param, currentLocation.y + currentLocation.dir.Item2 * instruction.param, currentLocation.dir),
						"N" => new coord(currentLocation.x, currentLocation.y + instruction.param, currentLocation.dir),
						"S" => new coord(currentLocation.x, currentLocation.y - instruction.param, currentLocation.dir),
						"E" => new coord(currentLocation.x + instruction.param, currentLocation.y, currentLocation.dir),
						"W" => new coord(currentLocation.x - instruction.param, currentLocation.y, currentLocation.dir),
						"L" => turn(new coord(currentLocation.x, currentLocation.y, currentLocation.dir), -instruction.param),
						"R" => turn(new coord(currentLocation.x, currentLocation.y, currentLocation.dir), instruction.param),
						_ => new coord(currentLocation.x, currentLocation.y, currentLocation.dir),
					});
		...
		private (int, int) MoveWaypoint(instruction instruction, (int, int) wp)
		{
			...
			return instruction.action switch
			{
				...
				("S") => (wp.Item1, wp.Item2 - instruction.param),
